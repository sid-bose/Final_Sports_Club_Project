...
$sql="SELECT id FROM single_creator WHERE sports_name='Batminton' AND flag_var='1'";
$result=$conn->query($sql);
...
if(!empty($event_id))
  {
     if($flag==true)
     {
           $sql="INSERT INTO batminton_reg (`event_id`,`player_name`) VALUES ('$event_id','$name')";
           if($conn1->query($sql))
           {
               $_SESSION['event_id']=$event_id;
               $_SESSION['player_name']=$name;
               echo "<script> window.location.assign('batminton_reg.php'); </script>";
