...
<head>
    ...
    <title>Table-tennis Player Details</title>
</head>
<body>
    
    <form action="tt_play_details.php" method="post">
    <p>Player 1 Name</p>
...
$sql="SELECT player_name FROM tt_reg WHERE event_id='$existing_id'";
...
if ($result->num_rows > 0) 
    ...
    if ($result->num_rows > 0) 
        ...
        {
            $sql="INSERT INTO tt_live (`player_1`, `player_2`) VALUES ('$name1','$name2')";
                 ...
                 if ($result->num_rows > 0) 
        {
            while($row = $result->fetch_assoc())
            {
                $_SESSION['tt_id']=$row['tt_id'];
            }
    
        }
                 
               
                 echo "<script> window.location.assign('tt_match.php'); </script>";
            }
            else{
