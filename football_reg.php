...
<section class="text-gray-400 bg-gray-900 body-font">
  <div class="container px-5 py-24 mx-auto flex flex-wrap items-center">
    <div class="lg:w-3/5 md:w-1/2 md:pr-16 lg:pr-0 pr-0">
      <h1 class="title-font font-medium text-3xl text-white">Football Registration Portal</h1>
    ...
    <div class="lg:w-2/6 md:w-1/2 bg-gray-800 bg-opacity-50 rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0">
      <h2 class="text-white text-lg font-medium title-font mb-5">Football Registration</h2>
      <form action="football_reg.php" method="post">
      ...
      <div class="relative mb-4">
        ...
        <input type="text" id="full-name" name="position" placeholder="Goalkeeper or Defenser or Offsensor or Center" class="w-full bg-gray-600 bg-opacity-20 focus:bg-transparent focus:ring-2 focus:ring-red-900 rounded border border-gray-600 focus:border-red-500 text-base outline-none text-gray-100 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
      </div>
...
$position=array('Goalkeeper','Defender','Offensive','Center');
for($i=0;$i<=3;$i++)
{
  if($pos==$position[$i])
  {
...
$sql1="SELECT * FROM football_reg WHERE team_name='$tname_a'";
...
$sql1="SELECT * FROM football_reg WHERE team_name='$tname_b'";
$result=$conn1->query($sql1);
if ($result->num_rows > 0)
{
...
if( !empty($email) && !empty($tname) && !empty($pos))
{
   if($flag==true)
   {
       ...
       if($flag3==false && $flag4==false)
        ...
        echo '<script>alert("position has to be either Goalkeeper,Defender,Offensive or Center")</script>';
       }
