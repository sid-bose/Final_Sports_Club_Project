...
<head>
    ...
    <title>Volleyball Team Details</title>
</head>
<body>
    
    <form action="voll_play_details.php" method="post">
    <p>Team A Name</p>
...
$sql="SELECT team_name FROM voll_reg WHERE event_id='$existing_id'";
...
if ($result->num_rows > 0) 
    ...
    if ($result->num_rows > 0) 
        ...
        {
            $sql="INSERT INTO voll_live (`team_a`, `team_b`) VALUES ('$namea','$nameb')";
                 ...
                 if ($result->num_rows > 0) 
        {
            while($row = $result->fetch_assoc())
            {
                $_SESSION['voll_id']=$row['voll_id'];
                 ...
                 echo "<script> window.location.assign('voll_match.php'); </script>";
            }
            else{
