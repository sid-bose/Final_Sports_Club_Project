...
$sql="SELECT id FROM double_creator WHERE sports_name='Volleyball' AND flag_var='1'";
$result=$conn->query($sql);
...
if ($result->num_rows > 0) 
    ...
    $sql="SELECT * FROM double_creator WHERE id='$event_id'";
    $result=$conn->query($sql);
    if ($result->num_rows > 0) 
    {
        while($row = $result->fetch_assoc())
        {
            ...
            $_SESSION['no_play_a']=$row['no_play_a'];
            $_SESSION['no_play_b']=$row['no_play_b'];
        }
    }
    ...
    if ($result->num_rows > 0) 
    {
        while($row = $result->fetch_assoc())
        {
           ...
           $real_tnameb=$row['tname_b'];
            
            break;
        }
    }
    $_SESSION['tname_a']=$real_tnamea;
    $_SESSION['tname_b']=$real_tnameb;
    $team_a_n=$_SESSION['tname_a'];
    $team_b_n=$_SESSION['tname_b'];
    $sql="SELECT * FROM voll_reg";
    ...
    if($result)
    {
      if($result->num_rows == 0)
      {
        ...
        $sql="INSERT INTO voll_reg (`event_id`,`player_name`,`email`,`team_name`,`pos`) VALUES ('$event_id','none1','none12@gmail.com','$team_a_n','bowler')";
        $result1=$conn1->query($sql);
        $sql="INSERT INTO voll_reg (`event_id`,`player_name`,`email`,`team_name`,`pos`) VALUES ('$event_id','none2','none34@gmail.com','$team_b_n','bowler')";
...
// $sql="SELECT Name FROM registration_details";
...
if(!empty($event_id))
  {
     if($flag==true)
     {
           $sql="INSERT INTO voll_reg (`event_id`,`player_name`) VALUES ('$event_id','$name')";
           if($conn1->query($sql))
           {
               $_SESSION['event_id']=$event_id;
               $_SESSION['player_name']=$name;
               echo "<script> window.location.assign('voll_reg.php'); </script>";
